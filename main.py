...
def generate_gemini_response(prompt: str) -> str:
    ...
    if OPENROUTER_API_KEY:
        try:
            logging.info("Attempting request with: OpenRouter (Gemini 2.0 Flash Exp)...")
            headers = {
                "Authorization": f"Bearer {OPENROUTER_API_KEY}",
                "HTTP-Referer": "https://t.me/Oiuhelper_bot",  # رابط البوت الخاص بك
                "X-Title": "AI Quiz Bot",
                "Content-Type": "application/json"
            }
            model_identifier = "google/gemini-2.0-flash-exp:free"  # ✅ النموذج المجاني الحديث
            ...
            logging.info("✅ Success with OpenRouter (Gemini 2.0 Flash Exp).")
            return result_text

        except Exception as e:
            logging.warning(f"❌ OpenRouter (Gemini 2.0 Flash Exp) failed: {e}")
        
    if OPENROUTER_API_KEY:
        try:
            logging.info("Attempting request with: OpenRouter (DeepSeek Chat v3)...")
            headers = {
                "Authorization": f"Bearer {OPENROUTER_API_KEY}",
                "HTTP-Referer": "https://t.me/Oiuhelper_bot",  # رابط البوت الخاص بك
                "X-Title": "AI Quiz Bot",
            ...
            model_identifier = "deepseek/deepseek-chat-v3-0324:free"  # ✅ مجاني وقوي
            ...
            response.raise_for_status()
            result_text = response.json()['choices'][0]['message']['content']
            logging.info("✅ Success with OpenRouter (DeepSeek Chat v3).")
            return result_text

        except Exception as e:
            logging.warning(f"❌ OpenRouter (DeepSeek Chat v3) failed: {e}")
...
def generate_quizzes_for_lesson(lesson_id):
    ...
    prompt = (
    "أنت مساعد تعليمي ذكي. مهمتك توليد 3 اختبارات قصيرة (Quiz) باللغة العربية بناءً على هذه البطاقات التعليمية المستخرجة من فيديو تعليمي.\n\n"
    "🎯 كل اختبار يجب أن يحتوي على 3 إلى 5 أسئلة، ومنوع بين:\n"
    "- أسئلة اختيار من متعدد (type: \"mcq\")\n"
    "- أسئلة صح أو خطأ (type: \"true_false\")\n"
    "- أسئلة إكمال الجملة (type: \"fill_blank\")\n\n"
    "كل سؤال يتضمن الحقول التالية:\n"
    "- \"type\": نوع السؤال (mcq / true_false / fill_blank)\n"
    "- \"question\": نص السؤال باللغة العربية\n"
    "- \"options\": قائمة بالخيارات المتاحة (4 عناصر دائمًا)\n"
    "- \"answer\": الخيار الصحيح كما هو موجود في options\n\n"
    "📘 البيانات المستخرجة:\n"
    f"{flashcards_json}\n\n"
    "📌 المطلوب: أرسل فقط قائمة JSON تحتوي على 3 مجموعات من الأسئلة، كل مجموعة عبارة عن قائمة أسئلتها الخاصة. ⚠️ لا تدمج كل الأسئلة في قائمة واحدة، بل اجعل الشكل النهائي بهذا الشكل:\n\n"
    "[\n"
    "  [\n"
    "    {\n"
    "      \"type\": \"mcq\",\n"
    ...
    "    {\n"
    "      \"type\": \"fill_blank\",\n"
    "      \"question\": \"أكمل الجملة: He can't ____ this.\",\n"
    "      \"options\": [\"do\", \"say\", \"know\", \"hide\"],\n"
    "      \"answer\": \"do\"\n"
    "    }\n"
    "  ],\n"
    "  [ ... مجموعة ثانية ... ],\n"
    "  [ ... مجموعة ثالثة ... ]\n"
    "]\n\n"
    "✅ لا تضف أي شرح خارجي أو تعليقات أو مقدمة.\n"
    "✅ فقط JSON خام صالح 100%."
)
    # توليد الرد
    ai_response = generate_gemini_response(prompt)
    raw_json = extract_json_from_string(ai_response)
